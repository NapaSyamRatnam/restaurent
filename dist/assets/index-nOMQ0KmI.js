...
CREATE POLICY "Public Location Control" ON public.locations FOR ALL USING (true);
  ...
  `;const l=document.getElementById("admin-update-details-btn");l&&(l.onclick=()=>po());const c=document.getElementById("admin-supabase-config-btn");c&&(c.onclick=()=>Ks());const d=document.getElementById("supabase-status-badge");d&&(d.onclick=()=>Ks());const h=document.getElementById("admin-add-item-btn");h&&(h.onclick=()=>we());const u=document.getElementById("admin-add-branch-btn");u&&(u.onclick=()=>pe());const p=document.getElementById("admin-add-reservation-btn");p&&(p.onclick=()=>Zt());const m=document.getElementById("tab-admin-items"),g=document.getElementById("tab-admin-locations"),b=document.getElementById("tab-admin-orders"),v=document.getElementById("tab-admin-reservations");function S(w){[m,g,b,v].forEach(x=>x&&x.classList.remove("active"));const A=document.getElementById("admin-tab-content");A&&(w==="items"?(m&&m.classList.add("active"),co(A)):w==="locations"?(g&&g.classList.add("active"),uo(A)):w==="orders"?(b&&b.classList.add("active"),ho(A)):w==="reservations"&&(v&&v.classList.add("active"),fo(A)))}m&&(m.onclick=()=>S("items")),g&&(g.onclick=()=>S("locations")),b&&(b.onclick=()=>S("orders")),v&&(v.onclick=()=>S("reservations")),S("items")}function co(t){const e=f.dishes;t.innerHTML=`
...
4. Click Run to create tables (profiles, dishes, locations, orders, reservations) and security policies!`)});const a=document.getElementById("seed-supabase-data-btn");a&&(a.onclick=async()=>{a.disabled=!0,a.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i> Uploading Data to Supabase...';const l=await ro(f.dishes,f.locations,f.orders,f.reservations);l&&l.success?(y(`Successfully seeded Supabase! (${l.seededDishes} Dishes, ${l.seededLocations} Branches, ${l.seededOrders} Orders, ${l.seededReservations} Bookings uploaded)`,"success",5e3),C(),await f.syncWithSupabase(),f.notify("VIEW_CHANGED","admin")):(y(`Seeding error: ${(l==null?void 0:l.error)||"Failed to populate Supabase"}`,"info"),a.disabled=!1,a.innerHTML='<i class="fa-solid fa-cloud-arrow-up"></i> Seed / Reflect All App Data to Supabase')});const o=document.getElementById("clear-supabase-credentials-btn");o&&(o.onclick=()=>{It("",""),C(),y("Disconnected from Supabase. Switched to Local Mode.","info"),f.notify("VIEW_CHANGED","admin")}),document.getElementById("save-supabase-credentials-btn").onclick=async()=>{const l=document.getElementById("supabase-url").value.trim(),c=document.getElementById("supabase-key").value.trim();if(!l||!c){y("Please enter both Supabase URL and Anon Key","info");return}It(l,c),C(),y("Saved Supabase configuration! Syncing live data...","success"),await f.syncWithSupabase(),f.notify("VIEW_CHANGED","admin")}}function po(){const t=f.currentUser||{name:"Syam Ratnam (Admin Manager)",email:"syamratnam123@gmail.com",phone:"+91 98480 12345"},e=`
  ...
  `;const s=document.getElementById("wishlist-explore-btn");s&&(s.onclick=()=>f.setView("menu")),document.querySelectorAll("[data-remove-wishlist]").forEach(i=>{i.onclick=()=>{const r=i.getAttribute("data-remove-wishlist");f.toggleWishlist(r),y("Item removed from Wishlist","info"),rs(t)}}),document.querySelectorAll("[data-wishlist-add]").forEach(i=>{i.onclick=()=>{const r=i.getAttribute("data-wishlist-add"),n=f.dishes.find(a=>a.id===r);n&&(f.addToCart(r,1),y(`Moved ${n.name} to Cart!`,"success"))}})}function He(){const t=document.getElementById("cart-drawer-body");if(!t)return;const e=f.getCartTotal(),s=f.cart;t.innerHTML=`
    ...
    `:`
      <div class="cart-items-list">
        ${s.map((d,h)=>{const u=f.dishes.find(p=>p.id===d.dishId);return u?`
    ...
    `}
  `;const i=document.getElementById("close-cart-drawer-btn"),r=document.getElementById("cart-overlay"),n=document.getElementById("cart-drawer"),a=()=>{n&&n.classList.add("hidden")};i&&(i.onclick=a),r&&(r.onclick=a);const o=document.getElementById("cart-start-ordering-btn");o&&(o.onclick=()=>{a(),f.setView("menu")}),document.querySelectorAll("[data-qty-change]").forEach(d=>{d.onclick=()=>{const h=parseInt(d.getAttribute("data-qty-change")),u=parseInt(d.getAttribute("data-delta")),p=f.cart[h];p&&(f.updateCartQty(h,p.qty+u),He())}});const l=document.getElementById("apply-coupon-btn");l&&(l.onclick=()=>{const d=document.getElementById("coupon-input-field");if(d){const h=f.applyCoupon(d.value);h.success?y(h.message,"success"):y(h.message,"info"),He()}});const c=document.getElementById("checkout-trigger-btn");c&&(c.onclick=()=>{a(),go()})}function go(){const t=f.getCartTotal(),e=f.profile;e.addresses.find(a=>a.isDefault)||e.addresses[0];const s=`
  ...
  `}function es(t){const e=f.profile,s=(e.rewardPoints/10).toFixed(2),i=e.addresses||[],r=e.paymentMethods||[],n=f.reservations.filter(u=>{const p=u.customerName&&(u.customerName.toLowerCase()===e.name.toLowerCase()||f.currentUser&&u.customerName.toLowerCase()===f.currentUser.name.toLowerCase()),m=u.phone&&e.phone&&u.phone.includes(e.phone.replace(/[^0-9]/g,"").slice(-8)),g=u.email&&(u.email.toLowerCase()===e.email.toLowerCase()||f.currentUser&&u.email.toLowerCase()===f.currentUser.email.toLowerCase());return p||m||g}),a=n.length>0?n:f.reservations;t.innerHTML=`
    <section class="container" style="padding-top: 3rem; padding-bottom: 4rem;">
  ...
  `;const o=document.getElementById("edit-profile-details-btn");o&&(o.onclick=()=>yo());const l=document.getElementById("redeem-points-btn");l&&(l.onclick=()=>wo()),document.querySelectorAll("[data-tab]").forEach(u=>{u.onclick=()=>{document.querySelectorAll("[data-tab]").forEach(v=>v.classList.remove("active")),u.classList.add("active");const p=u.getAttribute("data-tab"),m=document.getElementById("account-tab-addresses"),g=document.getElementById("account-tab-payments"),b=document.getElementById("account-tab-reservations");m&&(m.style.display=p==="addresses"?"block":"none"),g&&(g.style.display=p==="payments"?"block":"none"),b&&(b.style.display=p==="reservations"?"block":"none")}});const c=document.getElementById("book-new-table-user-btn");c&&(c.onclick=()=>f.setView("location")),t.querySelectorAll(".user-cancel-res-btn").forEach(u=>{u.onclick=async()=>{const p=u.getAttribute("data-user-res-id");confirm("Are you sure you want to cancel this table booking?")&&(await f.updateReservationStatus(p,"cancelled"),y(`Table reservation ${p} cancelled`,"info"))}});const d=document.getElementById("add-new-address-btn");d&&(d.onclick=()=>Js()),document.querySelectorAll(".edit-addr-btn").forEach(u=>{u.onclick=()=>{const p=u.getAttribute("data-edit-addr-id"),m=f.profile.addresses.find(g=>g.id===p);m&&Js(t,m)}}),document.querySelectorAll(".delete-addr-btn").forEach(u=>{u.onclick=()=>{const p=u.getAttribute("data-delete-addr-id");confirm("Delete this address?")&&(f.deleteAddress(p),y("Address removed","info"))}}),document.querySelectorAll(".set-default-addr-btn").forEach(u=>{u.onclick=()=>{const p=u.getAttribute("data-default-addr-id");f.setDefaultAddress(p),y("Default delivery address updated","success")}});const h=document.getElementById("add-new-payment-btn");h&&(h.onclick=()=>vo()),document.querySelectorAll(".delete-pay-btn").forEach(u=>{u.onclick=()=>{const p=u.getAttribute("data-delete-pay-id");confirm("Delete this payment method?")&&(f.deletePaymentMethod(p),y("Payment method removed","info"))}}),document.querySelectorAll(".set-primary-pay-btn").forEach(u=>{u.onclick=()=>{const p=u.getAttribute("data-primary-pay-id");f.setDefaultPayment(p),y("Primary payment method updated","success")}})}function yo(t){const e=f.profile,s=`
    <div class="form-group">
